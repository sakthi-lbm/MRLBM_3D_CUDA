#include "stat_header.cuh"

__device__ __forceinline__ void compute_forces_mass(const uint32_t dir_mask, const real *pop,
                                                    real &Fx, real &Fy, real &Fz, real &m)
{
    Fx = toReal(0.0);
    Fy = toReal(0.0);
    Fz = toReal(0.0);
    m = toReal(0.0);

    uint32_t mask = dir_mask;
    while (mask)
    {
        const int q = __ffs(mask) - 1;
        mask &= mask - 1;

        const real f = pop[q];
        Fx += f * toReal(d_cx[q]);
        Fy += f * toReal(d_cy[q]);
        Fz += f * toReal(d_cz[q]);
        m += f;
    }
}

__global__ void compute_incoming_force_mass_kernal(const nodeVar dMom, cylinderVar cylinder)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;

    real Fx_local = 0.0;
    real Fy_local = 0.0;
    real Fz_local = 0.0;
    real m_local = 0.0;

    if (tid < d_NB)
    {
        const size_t idx = cylinder.boundaryList[tid];

        const real rho = RHO_0 + dMom.rho[idx];
        const real ux = dMom.ux[idx];
        const real uy = dMom.uy[idx];
        const real uz = dMom.uz[idx];
        const real mxx = dMom.mxx[idx];
        const real myy = dMom.myy[idx];
        const real mzz = dMom.mzz[idx];
        const real mxy = dMom.mxy[idx];
        const real mxz = dMom.mxz[idx];
        const real myz = dMom.myz[idx];

        real pop[Q];
        pop_reconstruction(rho, ux, uy, uz, mxx, myy, mzz, mxy, mxz, myz, pop);

        const uint32_t incoming_mask = cylinder.incomingMask[tid];
        compute_forces_mass(incoming_mask, pop, Fx_local, Fy_local, Fz_local, m_local);
    }
    else if (tid < d_NB + d_NB_FLUID)
    {
        int fid = tid - d_NB;
        const size_t idx = cylinder.bcfluidList[fid];

        const real rho = RHO_0 + dMom.rho[idx];
        const real ux = dMom.ux[idx];
        const real uy = dMom.uy[idx];
        const real uz = dMom.uz[idx];
        const real mxx = dMom.mxx[idx];
        const real myy = dMom.myy[idx];
        const real mzz = dMom.mzz[idx];
        const real mxy = dMom.mxy[idx];
        const real mxz = dMom.mxz[idx];
        const real myz = dMom.myz[idx];

        real pop[Q];
        pop_reconstruction(rho, ux, uy, uz, mxx, myy, mzz, mxy, mxz, myz, pop);

        const size_t nodeTag = dMom.nodeType[idx] - BCFLUID_NODE;
        const uint32_t incoming_mask = d_incomingMask_bcfluid[nodeTag];
        compute_forces_mass(incoming_mask, pop, Fx_local, Fy_local, Fz_local, m_local);
    }

    __shared__ real sFx[BLOCK_NODES];
    __shared__ real sFy[BLOCK_NODES];
    __shared__ real sFz[BLOCK_NODES];
    __shared__ real sm[BLOCK_NODES];

    const unsigned int tx = threadIdx.x;

    sFx[tx] = Fx_local;
    sFy[tx] = Fy_local;
    sFz[tx] = Fz_local;
    sm[tx] = m_local;

    __syncthreads();

    for (int stride = blockDim.x / 2; stride > 0; stride >>= 1)
    {
        if (tx < stride)
        {
            sFx[tx] += sFx[tx + stride];
            sFy[tx] += sFy[tx + stride];
            sFz[tx] += sFz[tx + stride];
            sm[tx] += sm[tx + stride];
        }

        __syncthreads();
    }

    if (tx == 0)
    {
        atomicAdd(&d_TotalFx, sFx[0]);
        atomicAdd(&d_TotalFy, sFy[0]);
        atomicAdd(&d_TotalFz, sFz[0]);
        atomicAdd(&d_Totalm, sm[0]);
    }
}

__global__ void compute_outgoing_force_mass_kernal(const nodeVar dMom, cylinderVar cylinder)
{
    int tid = threadIdx.x + blockIdx.x * blockDim.x;

    real Fx_local = 0.0;
    real Fy_local = 0.0;
    real Fz_local = 0.0;
    real m_local = 0.0;

    if (tid < d_NB)
    {
        const size_t idx = cylinder.boundaryList[tid];

        const real rho = RHO_0 + dMom.rho[idx];
        const real ux = dMom.ux[idx];
        const real uy = dMom.uy[idx];
        const real uz = dMom.uz[idx];
        const real mxx = dMom.mxx[idx];
        const real myy = dMom.myy[idx];
        const real mzz = dMom.mzz[idx];
        const real mxy = dMom.mxy[idx];
        const real mxz = dMom.mxz[idx];
        const real myz = dMom.myz[idx];

        real pop[Q];
        pop_reconstruction(rho, ux, uy, uz, mxx, myy, mzz, mxy, mxz, myz, pop);

        const uint32_t outgoing_mask = cylinder.outgoingMask[tid];
        compute_forces_mass(outgoing_mask, pop, Fx_local, Fy_local, Fz_local, m_local);
    }
    else if (tid < d_NB + d_NB_FLUID)
    {
        int fid = tid - d_NB;
        const size_t idx = cylinder.bcfluidList[fid];

        const real rho = RHO_0 + dMom.rho[idx];
        const real ux = dMom.ux[idx];
        const real uy = dMom.uy[idx];
        const real uz = dMom.uz[idx];
        const real mxx = dMom.mxx[idx];
        const real myy = dMom.myy[idx];
        const real mzz = dMom.mzz[idx];
        const real mxy = dMom.mxy[idx];
        const real mxz = dMom.mxz[idx];
        const real myz = dMom.myz[idx];

        real pop[Q];
        pop_reconstruction(rho, ux, uy, uz, mxx, myy, mzz, mxy, mxz, myz, pop);

        const size_t nodeTag = dMom.nodeType[idx] - BCFLUID_NODE;
        const uint32_t outgoing_mask = d_outgoingMask_bcfluid[nodeTag];
        compute_forces_mass(outgoing_mask, pop, Fx_local, Fy_local, Fz_local, m_local);
    }

    __shared__ real sFx[BLOCK_NODES];
    __shared__ real sFy[BLOCK_NODES];
    __shared__ real sFz[BLOCK_NODES];
    __shared__ real sm[BLOCK_NODES];

    const unsigned int tx = threadIdx.x;

    sFx[tx] = Fx_local;
    sFy[tx] = Fy_local;
    sFz[tx] = Fz_local;
    sm[tx] = m_local;

    __syncthreads();

    for (int stride = blockDim.x / 2; stride > 0; stride >>= 1)
    {
        if (tx < stride)
        {
            sFx[tx] += sFx[tx + stride];
            sFy[tx] += sFy[tx + stride];
            sFz[tx] += sFz[tx + stride];
            sm[tx] += sm[tx + stride];
        }

        __syncthreads();
    }

    if (tx == 0)
    {
        atomicAdd(&d_TotalFx, -sFx[0]);
        atomicAdd(&d_TotalFy, -sFy[0]);
        atomicAdd(&d_TotalFz, -sFz[0]);
        atomicAdd(&d_Totalm, -sm[0]);
    }
}

void launch_incoming_force_kernal(const nodeVar &dMom, const cylinderVar &cylinder, const int iter)
{
    if (iter >= STAT_START && iter <= STAT_END)
    {
        real zero = 0.0;
        checkCudaErrors(cudaMemcpyToSymbol(d_TotalFx, &zero, sizeof(real)));
        checkCudaErrors(cudaMemcpyToSymbol(d_TotalFy, &zero, sizeof(real)));
        checkCudaErrors(cudaMemcpyToSymbol(d_TotalFz, &zero, sizeof(real)));
        checkCudaErrors(cudaMemcpyToSymbol(d_Totalm, &zero, sizeof(real)));

        const size_t FORCE_GRID = (NB + NB_FLUID + BLOCK_NODES - 1) / BLOCK_NODES;
        const dim3 force_grid(FORCE_GRID);

        compute_incoming_force_mass_kernal<<<force_grid, BLOCK_NODES>>>(dMom, cylinder);
        checkKernelExecution();
    }
}

void launch_outgoing_force_kernal(const nodeVar &dMom, const cylinderVar &cylinder, const int iter)
{
    if (iter >= STAT_START && iter <= STAT_END)
    {
        const size_t FORCE_GRID = (NB + NB_FLUID + BLOCK_NODES - 1) / BLOCK_NODES;
        const dim3 force_grid(FORCE_GRID);

        compute_outgoing_force_mass_kernal<<<force_grid, BLOCK_NODES>>>(dMom, cylinder);
        checkKernelExecution();
    }
}